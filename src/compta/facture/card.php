 ...
 * ***********************************************************************linea 5709 se inserta el codigo de factronica ******************
...
########################################################################################
# FACTRONICA => INICIO => BOTON ENVIAR AL SII CHILE
########################################################################################
if(isset($_GET["facid"])){
	$id_doc=$_GET["facid"];
...
$sql="SELECT trackid from ".MAIN_DB_PREFIX."facture_extrafields where fk_object='$id_doc'";
$result = $db->query($sql);
if ($result) {
	$num = $db->num_rows($result);
	$i = 0;
...
function EnviarSiiChile(facid){
var vars='facid='+parseInt(facid);
alert(vars);
ajax_EnviarSiiChile=objetoAjax();
ajax_EnviarSiiChile.open('GET','enviar_json.php',true);
ajax_EnviarSiiChile.onreadystatechange=function() {
	if (ajax_EnviarSiiChile.readyState==4) {
		respuesta = ajax_EnviarSiiChile.responseText;
	}
  }
  ajax_EnviarSiiChile.setRequestHeader('Content-Type','application/x-www-form-urlencoded');
  ajax_EnviarSiiChile.send(vars);
}
function EnviarSiiChileGet(facid){
location.href='enviar_json.php?facid='+$id_doc;
}
</script>";
if(   ($object->statut == Facture::STATUS_VALIDATED || $object->statut == Facture::STATUS_CLOSED) ) {
if(!$trackid_sii>0 and $id_doc>0){
	$accion_btnenviarsii="javascript:EnviarSiiChileGet('$id_doc')";
	print dolGetButtonAction("ENVIAR SII CHILE",'','default',$accion_btnenviarsii,'',true,$params);
}
}
########################################################################################
# FACTRONICA => FINAL => BOTON ENVIAR AL SII CHILE
########################################################################################
			// Validate
